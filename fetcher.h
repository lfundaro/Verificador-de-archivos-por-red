...
/**
 Descarga las paginas HTTP que se encuentran en la lista 'url_list'. Modifica el valor de 'nurls' para contener el número de elementos en la lista 'url_list'
...
/**
  Descarga una página HTTP en el bufer '*(pg_ptr)' desde la direccion almacenada en url->netInfo. Esta función abre la conección y construye la petición HTTP GET, llama a una función auxiliar para hacer la descarga.
...
/**
   ...
   \param url_header %URL que fue solicitado y que será concatenado al principio del contenido a descargar.

   \param url_header_sz Tamaño en bytes del %URL a concatenar.

   \return Código de terminación. Cero significa exitoso.
*/
int download_page(char** pg_ptr, int sock_des,char* url_header, int url_header_sz);



/**
   Descarga el header de una respuesta HTTP, usando el socket apuntado por 'sock_des', en un buffer y devuelve el apuntador del buffer.
